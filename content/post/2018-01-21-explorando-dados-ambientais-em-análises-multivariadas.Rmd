...
Para acompanhar este roteiro é preciso já entender um pouco do funcionamento da linguagem R.

As análises exploratórias de dados são imprescindíveis no início de qualquer análises de dados, principalmente em dados multidimensionais. É quando o analista começa a tomar familiaridade com os dados e a entender melhor suas estruturas e padrões.
...
```{r, echo=F}
kable(head(env), caption="Visualização das primeiras linhas dos dados ambientais. A coluna X identifica o local de coleta e cada coluna corresponde à uma variável ambiental medida. Para mais informações sobre os dados ver em Borcard et al. (2011).")
```
